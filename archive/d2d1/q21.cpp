...
struct lazySegtree {
  ...
  int n;
  int s;
  lazySegtree(int a_len) {  // change if type updated
    n = a_len;
    s = 1;
    while (s < n) {
      s = s << 1;
...
struct Update1 {
  ...
  void apply(Node1 &a, int start, int end) {  // apply update to given node
    if (val % 2) a.val = (end - start + 1) - a.val;     // may change
  }
...
signed main() {
  ...
  lazySegtree<Node1, Update1> sx(n);

  while (q--) {
    int t, x, y;
    cin >> t >> x >> y;
    if (t) {
      cout << sx.make_query(x - 1, y - 1).val << "\n";
    }
    else {
      sx.make_update(x - 1, y - 1, 1);
    }
  }
