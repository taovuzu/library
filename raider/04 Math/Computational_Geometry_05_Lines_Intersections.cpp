...
// Define a point as a complex number
typedef complex<double> Point;

// Cross product of two vectors
// Determines orientation: ccw (>0), collinear (0), cw (<0)
double crossProduct(Point a, Point b) {
    return (conj(a) * b).imag();
}


// Function to determine the orientation of three points (Counter-Clockwise Test)
int ccw(Point a, Point b, Point c) {
    ...
    if (v1.real() * v2.real() < -EPS || v1.imag() * v2.imag() < -EPS) return -1; // Collinear but opposite direction
    if (norm(v1) < norm(v2) - EPS) return +1; // Collinear but c is beyond b
    return 0; // Collinear and on segment
}

// Function to check if two segments intersect
bool doSegmentsIntersect(Point a, Point b, Point c, Point d, Point &intersection) {
    double d1 = crossProduct(a - b, d - c); // Determinant
    double d2 = crossProduct(a - c, d - c);
    double d3 = crossProduct(a - b, a - c);
    
    if (fabs(d1) < EPS) return false; // Parallel or identical
    
    double t1 = d2 / d1;
    double t2 = d3 / d1;
    
    intersection = a + (b - a) * t1; // Compute intersection point
    
    return (t1 >= -EPS && t1 <= 1 + EPS && t2 >= -EPS && t2 <= 1 + EPS);
}

// Function to check if two segments intersect using CCW test
...
void testCCW() {
    ...
    cout << ccw(a, b, Point(20, 0)) << "\n";   // +1 => Collinear beyond b
    cout << ccw(a, b, Point(20, -10)) << "\n"; // -1 => CW
    cout << ccw(a, b, Point(-20, 0)) << "\n";  // -1 => Collinear before a
    cout << ccw(a, b, a) << "\n";             // 0 => Collinear at a
    cout << ccw(a, b, b) << "\n";             // 0 => Collinear at b
    cout << ccw(a, b, (a + b) / 2.0) << "\n"; // 0 => Collinear between a and b
}
...
int main() {
    ...
    if (doSegmentsIntersect(a, b, c, d, intersection)) {
        cout << "Segments Intersect at: (" << intersection.real() << ", " << intersection.imag() << ")\n";
    } else {
